...
if (isset($_GET['specialty_id'])) {
    ...
    $stmt = $mysqli->prepare("SELECT u.id, u.nombre, u.apellido, m.id_perfil, rol, m.foto_perfil FROM usuarios u JOIN medicos m ON m.id_medico = u.id WHERE m.id_especialidad = ? AND u.rol <> 5;");
    if ($stmt) {
        ...
        while ($row = $result->fetch_assoc()) {
            $medicos[] = ['nombre' => $row['nombre'], 'apellido' => $row['apellido'] , 'id_medico' => $row['id'], "foto" => $row['foto_perfil'], "perfilMed" => $row['id_perfil']];
        }
